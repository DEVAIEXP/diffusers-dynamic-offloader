...
| Native Ubuntu modular, DDO `one_shot_fast` vs best official Diffusers leaf group offload | `35.6s` total, `24.26s` denoise, `1.42 GB` RAM | `38.0s` total, `13.00s` denoise, `1.45 GB` RAM | DDO is `6.7%` slower than leaf total; it is `1.3%` faster than the refreshed block-level total (`38.5s`) | `46.4%` lower denoise time (`1.87x` faster) vs leaf; `52.1%` lower (`2.09x` faster) vs block | Similar recorded RAM | Latest native Ubuntu DDO run improved enough to edge past block-level Diffusers overall, but leaf-level Diffusers remains the fastest one-shot total because setup cost is tiny. |
...
| WSL Ubuntu | `diffusers_leaf_offload_compat` | real | no Windows standby purge | not completed in the WSL run | `0.011s` Diffusers group-offload setup | not completed | not completed | not completed | not completed | not captured | not captured | Historical WSL-only result: official Diffusers `leaf_level` group offload loaded and prepared, then the WSL process exited/crashed immediately after `Starting denoise loop` before the first denoise callback. This is separate from the completed Native Ubuntu leaf run below. |
| Native Ubuntu | `auto -> one_shot_fast` | real | none | `10.6s` | `11.92s` DDO setup, full-pin fast path | `13.00s` | `26.5s` | `0.6s` | `38.0s` | `6.27 GB` | `1.45 GB` | Latest native Linux DDO run after runner/planner updates. Denoise steps stayed around `1.57s`; transformer setup is still the main remaining cost, but total now slightly beats the refreshed block-level Diffusers run. |
...
| Native Ubuntu | `diffusers_offload_compat` | real | none | `8.4s` | `0.015s` Diffusers group-offload setup | `27.14s` | `29.3s` | `0.6s` | `38.5s` | `5.96 GB` | `1.53 GB` | Refreshed official Diffusers `block_level` group offload run. Its setup remains almost free, but the denoise loop is about `2.1x` slower than DDO and the latest total is now slightly slower than DDO. |
